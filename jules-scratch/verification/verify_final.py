...
def run():
    with sync_playwright() as p:
        ...
        # Give it a moment to fetch suggestions
        time.sleep(2) # Increased delay

        # 3. Wait for suggestions to appear and click the first one.
        page.wait_for_selector('.suggestion-item', timeout=5000) # shorter timeout for faster failure
        page.click('.suggestion-item')

        # 4. Wait for the weather to update.
        page.wait_for_load_state('networkidle')
        time.sleep(1) # a little extra time for rendering
